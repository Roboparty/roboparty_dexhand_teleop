...
class DexPilotOptimizer(Optimizer):
    ...
    def __init__(
        ...
        # For one link used in multiple vectors, e.g. hand palm, we do not want to compute it multiple times
        self.computed_link_names = list(set(target_origin_link_names).union(set(target_task_link_names)))
        self.origin_link_indices = np.asarray(
            [self.computed_link_names.index(name) for name in target_origin_link_names], dtype=int
        )
        self.task_link_indices = np.asarray(
            [self.computed_link_names.index(name) for name in target_task_link_names], dtype=int
        )
    ...
    def get_objective_function(self, target_vector: np.ndarray, fixed_qpos: np.ndarray, last_qpos: np.ndarray):
        ...
        weight = np.concatenate(
            [weight, np.ones(self.num_fingers, dtype=np.float32) * (len_proj + self.num_fingers)]
        )
        ...
        reference_vec = np.asarray(reference_vec, dtype=np.float64)
        ...
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            error = robot_vec - reference_vec
            vec_dist = np.linalg.norm(error, axis=1)
            quadratic = vec_dist < self.huber_delta
            losses = np.where(
                quadratic,
                0.5 * np.square(vec_dist) / self.huber_delta,
                vec_dist - 0.5 * self.huber_delta,
            )
            result = float(np.sum(losses * weight) / robot_vec.shape[0])
            ...
            if grad.size > 0:
                ...
                # Note: the joint order in this jacobian is consistent pinocchio
                jacobians = np.stack(jacobians, axis=0)
                distance_scale = np.divide(
                    1.0,
                    vec_dist,
                    out=np.zeros_like(vec_dist),
                    where=vec_dist > 1e-12,
                )
                distance_scale[quadratic] = 1.0 / self.huber_delta
                grad_vec = (
                    error
                    * distance_scale[:, None]
                    * (weight / robot_vec.shape[0])[:, None]
                )
                grad_pos = np.zeros_like(body_pos)
                np.add.at(grad_pos, self.origin_link_indices, -grad_vec)
                np.add.at(grad_pos, self.task_link_indices, grad_vec)
                ...
                grad_qpos = np.einsum("li,lij->j", grad_pos, np.asarray(jacobians))
