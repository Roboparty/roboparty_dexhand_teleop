...
class RetargetingConfig:
    ...
    def build(self) -> SeqRetargeting:
        ...
        if self.add_dummy_free_joint:
            raise ValueError("RP_Hand retargeting does not support a dummy free joint")
        robot = RobotWrapper(self.urdf_path)
...
def parse_mimic_joint(urdf_path: Union[str, Path]) -> Tuple[bool, List[str], List[str], List[float], List[float]]:
    mimic_joint_names = []
    source_joint_names = []
    multipliers = []
    offsets = []
    ...
    for joint in robot.findall("joint"):
        ...
        if mimic is not None:
            mimic_joint_names.append(str(joint.get("name")))
            source_joint_names.append(str(mimic.get("joint")))
            multipliers.append(float(mimic.get("multiplier", "1.0")))
            offsets.append(float(mimic.get("offset", "0.0")))
